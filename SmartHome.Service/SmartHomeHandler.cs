...
namespace SmartHome.Service
{
    public class SmartHomeHandler : ISmartHomeHandler, IDisposable
    {
        ...
        public SmartHomeHandler(string configFilename, string libDirname)
        {
            ...
            this.eventsManager = SIManager.Container.GetInstance<ISaveEventsManager>();
        }
        ...
        private void InitHomeObjects(List<Type> allowTypes)
        {
            ...
            foreach (XElement elem in configs)
            {
                ...
                try
                {
                    ...
                    config.TypeID = SIManager.Container.GetInstance<IDeviceTypeRepository>().Add(new DeviceType { Name = currentType.FullName }).ID;
                    ...
                    switch (elemName)
                    {
                        case "controller":
                            ...
                            controller.ID = SIManager.Container.GetInstance<IDeviceRepository>().Add(new Device { Name = controller.Name, Type = new DeviceType { ID = controller.TypeID } }).ID;
                            this.Controllers.Add(controller);
                            break;
                        case "sensor":
                            this.CheckConfigName<ISensor>(config.Name, this.Sensors, elemName);
                            ISensor sensor = (ISensor)config;
                            sensor.ID = SIManager.Container.GetInstance<ISensorRepository>().Add(new Sensor { Name = sensor.Name, Type = new DeviceType { ID = sensor.TypeID } }).ID;
                        ...
                        case "trigger":
                            ...
                            { 
                                Name = trigger.Name, 
                                Type = new DeviceType { ID = trigger.TypeID }, 
